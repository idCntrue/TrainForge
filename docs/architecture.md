...
前端不维护独立的移动端应用或第二套路由。`useMobileViewport` 在 `900px` 断点下切换展示结构：桌面侧栏变为固定底部导航，表格变为卡片，抽屉变为全宽，标注工作台改为纵向触控布局。页面仍复用相同的 repository、API schema、导航历史与保存冲突处理，因此视口变化不会形成第二套业务逻辑。
