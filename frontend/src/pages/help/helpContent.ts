...
export const helpChapters: HelpChapter[] = [
  ...
  { id: 'deployment', title: '部署与运维', summary: '本地运行、存储、备份和后续云端部署接口。', sections: [
    { id: 'bounded-training', title: 'CPU 安全模式与资源保底', content: ['无 GPU 云服务器默认启用 CPU 安全模式：检测任务默认 Batch 2、上限 4；分割任务默认 Batch 1、上限 2；图像尺寸默认 320、上限 640；训练 worker 为 0，CPU 线程数为 4。超过上限的请求会在创建训练记录前被拒绝。', 'API 容器默认使用 API_MEMORY_LIMIT=10g、API_CPU_LIMIT=6、API_SHM_SIZE=2gb、API_PIDS_LIMIT=256。资源耗尽导致进程退出时，训练任务会显示容器内存或进程资源达到限制，而不是拖垮整台服务器。', '训练前必须同时保留至少 10 GiB 和 10% 可用磁盘，否则系统拒绝启动训练。当前 40 GB 系统盘仍应尽快扩容，容器限制不能解决训练制品持续增长。'] },
