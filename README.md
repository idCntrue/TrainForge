...
<p align="center">
  ...
  <a href="CHANGELOG.md"><img src="https://img.shields.io/badge/version-0.2.0-2563EB.svg" alt="Version 0.2.0"></a>
...
- 训练页面支持空闲时手动释放平台自身资源：清理缩略图、过期上传暂存和临时文件，回收当前 API 进程可释放的 Python/CUDA 缓存，并返回 Windows 或 Linux 的最新资源快照。活动训练和推理期间会拒绝执行。
- 记录 cgroup 内存限制、当前值、峰值和本次运行的 OOM kill 增量，区分已确认 OOM 与原因未确认的外部 `SIGKILL`。
- Windows 训练启动前会同时检查提交内存和物理内存；模型加载后及每轮开始前只使用提交内存作为停止条件，避免把训练自身的正常物理内存占用误判为故障。失败面板会展示 `0xC0000005`、剩余内存及 `LeASPac` 占用证据。
- test 集评估、数据质量报告、类别指标和最佳权重恢复评估。
- PT/ONNX 制品管理、环境真实路径与哈希展示、opset 17 导出、一对一实例一致性门禁、失败样本对比图、发布和归档。
- PT/CUDA 与 ONNX/CPU 的图片、批量图片和视频异步推理；支持正式发布模型、候选/阻断模型及可复用的上传测试模型。
...
8. 发布通过门禁的模型，并在推理工作台验证图片或视频；也可选择“候选 / 阻断”或上传 `.pt`/`.onnx` 做发布前测试，测试结果不代表正式发布。
