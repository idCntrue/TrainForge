...
def append_videos_to_batch(
    ...
    *,
    ...
    staging_root = _managed_directory(
        storage_root / ".operations" / "video-append", uuid.uuid4().hex
    )
    staged_videos_dir = staging_root / "videos"
    staged_frames_dir = staging_root / "frames"
    staged_videos_dir.mkdir(parents=True)
    staged_frames_dir.mkdir(parents=True)

    imported: list[tuple[VideoAsset, Path, Path]] = []
    created_frames: list[tuple[FrameAsset, Path, Path]] = []
    ...
    try:
        for source_path in _video_sources(source_dir):
            ...
            staged_video = staged_videos_dir / f"{content_hash}{source_path.suffix.lower()}"
            shutil.copyfile(source_path, staged_video)
            if sha256_file(staged_video) != content_hash:
                raise OSError(f"copied video hash mismatch: {source_path.name}")
            destination = videos_dir / staged_video.name
            video = VideoAsset(
                id=video_id,
            ...
            imported.append((video, staged_video, destination))

            frames = extractor(
                staged_video,
                staged_frames_dir / video_id,
                collection_id,
                video.id,
                interval,
                quality,
            ...
            for frame in frames:
                ...
                final_frame = candidates_dir / f"{video.id}-{frame.sha256[:16]}{frame.path.suffix.lower()}"
                storage_key = _storage_key(final_frame, storage_root)
                record = FrameAsset(
                    id=f"frame-{identity}",
                    batch_id=batch_id,
                    video_id=video.id,
                ...
                created_frames.append((record, frame.path, final_frame))
                created_for_video += 1
        ...
        if imported:
            ...
            for _, staged, destination in imported:
                destination.parent.mkdir(parents=True, exist_ok=True)
                staged.replace(destination)
                moved_paths.append(destination)
            for _, staged, destination in created_frames:
                destination.parent.mkdir(parents=True, exist_ok=True)
                staged.replace(destination)
                moved_paths.append(destination)

            with session_scope(registry) as session:
                session.add_all([record for record, _, _ in imported])
                session.flush()
                session.add_all([record for record, _, _ in created_frames])
        ...
        frame_ids = [record.id for record, _, _ in created_frames]
        video_ids = [record.id for record, _, _ in imported]
        if frame_ids or video_ids:
            with session_scope(registry) as session:
                if frame_ids:
                    session.execute(delete(FrameAsset).where(FrameAsset.id.in_(frame_ids)))
                if video_ids:
                    session.execute(delete(VideoAsset).where(VideoAsset.id.in_(video_ids)))
        for path in reversed(moved_paths):
            path.unlink(missing_ok=True)
        if original_manifest is None:
            manifest_path.unlink(missing_ok=True)
        else:
            temporary = manifest_path.with_suffix(manifest_path.suffix + ".restore")
